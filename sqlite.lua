-- sqlite.lua
-- Declares the cSQLite class representing a single SQLite database connection.

local cSQLite = {}
cSQLite.__index = cSQLite

-- Creates the table of specified names and columns.
-- If the table exists, missing columns are added and existing data is preserved.
function cSQLite:CreateDBTable(a_TableName, a_Columns)
	...
	if (ExecResult ~= sqlite3.OK) then
		LOGWARNING(self.PluginPrefix .. "Cannot create database table " .. a_TableName .. ": " .. ExecResult)
		LOGWARNING(self.PluginPrefix .. "Command: \"" .. sql .. "\".")
		return false
	end
	
	-- Check whether each column exists.
  	-- Remove all existing columns from a_Columns.
	local RemoveExistingColumn = function(a_Values)
		if (a_Values.name ~= nil) then
			...
			for j = 1, #a_Columns do
				...
				-- If it's a match, remove from a_Columns.
	...
	-- a_Columns now contains only those columns that are missing.
	if (#a_Columns > 0) then
...
-- Executes the SQL statement, substituting "?" with the specified parameters.
-- Calls a_Callback for each row; receives a dictionary table containing the row values (stmt:nrows()).
